...
JNIEXPORT void JNICALL Java_jvips_VipsNative_init
    ...
    ASSIGN_CLASS_GLOBAL("[J", longArrayClass);
    ASSIGN_CLASS_GLOBAL("[java/nio/ByteBuffer", bbArrayClass);
...
jint getInt(JNIEnv* env, jobject value){
    return env->GetIntField(value, javaInteger.value);
}

jbyte getByte(JNIEnv* env, jobject value){
    return env->GetByteField(value, javaByte.value);
}

jlong getLong(JNIEnv* env, jobject value){
    return env->GetLongField(value, javaLong.value);
}

jdouble getDouble(JNIEnv* env, jobject value){
    return env->GetDoubleField(value, javaDouble.value);
}

jboolean getBoolean(JNIEnv* env, jobject value){
    return env->GetBooleanField(value, javaBoolean.value);
}
...
void unrefArea(void* areaType){
    vips_area_unref(VIPS_AREA(areaType));
}
...
JNIEXPORT jlong JNICALL Java_jvips_VipsNative_callNative
    ...
    std::vector<VipsArea*> allocations;
        ...
        if (argument.valueNotNull) {
            switch (argument.type) {
                ...
                case jvips_VipsNative_IMAGE_ARRAY: {
                    ...
                    if(unsafe && IS_TYPE(argument.value, longArrayClass)) {
                        auto length = env->GetArrayLength((jlongArray) argument.value);
                        auto longs = env->GetLongArrayElements((jlongArray) argument.value, 0);
                        for (int j = 0; j < length; j++) {
                            jlong value = longs[j];
                            if (value != 0) {
                                VipsImage *image = (VipsImage *) (void *) value;
                                if (VIPS_IS_IMAGE(image)) {
                                    images.push_back(image);
                                }
                            }
                        }
                    } else if(IS_TYPE(argument.value, bbArrayClass)) {
                        auto length = env->GetArrayLength((jlongArray) argument.value);
                        for (int j = 0; j < length; j++) {
                            jobject value = env->GetObjectArrayElement((jobjectArray) argument.value, j);
                            if (NOT_NULL_CHECK(value) && env->IsInstanceOf(value, ByteBuffer_class)) {
                                void* address = env->GetDirectBufferAddress(value);
                                if(address != nullptr){
                                    VipsImage *image = (VipsImage *) address;
                                    if (VIPS_IS_IMAGE(image)) {
                                        images.push_back(image);
                                    }
                                }
                    ...
                    std::unique_ptr<VipsArrayImage, decltype(&unrefArea)> vipsArea(
                    vips_array_image_new(images.data(), images.size()), &unrefArea);
                    g_value_init(&gvalue, VIPS_TYPE_ARRAY_IMAGE);
                    g_value_set_object(&gvalue, vipsArea.get());
                    g_object_set_property(G_OBJECT(op), argument.name->c_str(), &gvalue);
                    g_value_unset(&gvalue);
                ...
                case jvips_VipsNative_VIPS_ARRAY_DOUBLE: {
                    if(IS_TYPE(argument.value, doubleArrayClass)) {
                        auto length = env->GetArrayLength((jdoubleArray) argument.value);
                        auto doubles = env->GetDoubleArrayElements((jdoubleArray) argument.value, 0);
                        std::unique_ptr<VipsArrayDouble, decltype(&unrefArea)> vipsArea(
                        vips_array_double_new(doubles, length), &unrefArea);
                ...
                case jvips_VipsNative_VIPS_ARRAY_INT: {
                    if(IS_TYPE(argument.value, intArrayClass)) {
                        auto length = env->GetArrayLength((jintArray) argument.value);
                        auto doubles = env->GetIntArrayElements((jintArray) argument.value, 0);
                        std::unique_ptr<VipsArrayInt, decltype(&unrefArea)> vipsArea(
                                vips_array_int_new(doubles, length), &unrefArea);
